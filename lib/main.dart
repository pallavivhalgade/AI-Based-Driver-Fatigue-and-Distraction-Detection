    ...
    return MaterialApp(
      ...
      theme: ThemeData(
        ...
        colorScheme: ColorScheme.fromSeed(
          seedColor: const Color(0xFF27D77F),
          brightness: Brightness.dark,
        ),
        useMaterial3: true,
      ),
    ...
    return const Scaffold(
      body: Center(
        child: Column(
          ...
          children: [
            Icon(Icons.shield_rounded, size: 72, color: Color(0xFF27D77F)),
            SizedBox(height: 18),
            Text(
              'DRIVERGUARD AI',
              style: TextStyle(
          ...
          slivers: [
              ...
              sliver: SliverToBoxAdapter(
                child: Row(
                  children: [
                    CircleAvatar(
                      radius: 22,
                      backgroundColor: Color(0xFF0E2739),
                      child: Icon(Icons.shield_rounded, color: Color(0xFF27D77F)),
                    ),
                    SizedBox(width: 12),
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          Text('DriverGuard', style: TextStyle(fontSize: 20, fontWeight: FontWeight.w900)),
                          SizedBox(height: 2),
                          Text('AI SAFETY SYSTEM', style: TextStyle(fontSize: 10, color: Color(0xFF7F9BB1), fontWeight: FontWeight.w800, letterSpacing: 1.1)),
                        ],
                      ),
                    ),
                    Icon(Icons.security_rounded, color: Color(0xFF9BB0C1)),
                  ],
                ),
              ),
            ),
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    gradient: const LinearGradient(
                      ...
                      colors: [Color(0xFF12304A), Color(0xFF0B1B2A)],
                    ),
                    border: Border.all(color: const Color(0xFF214A68)),
                  ),
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Row(
                        children: [
                          Container(
                            ...
                            decoration: BoxDecoration(
                              color: const Color(0xFF1BCB78).withOpacity(.12),
                          ...
                          const Spacer(),
                          const Text('ON-DEVICE AI', style: TextStyle(fontSize: 10, color: Color(0xFF7F9BB1), fontWeight: FontWeight.w800)),
                        ],
                      ),
                      const SizedBox(height: 24),
                      const Text('Drive with confidence.', style: TextStyle(fontSize: 28, fontWeight: FontWeight.w800)),
                      const SizedBox(height: 8),
                      const Text('Real-time monitoring for eye closure, yawning and head position.', style: TextStyle(color: Color(0xFF9BB0C1), height: 1.45)),
                      ...
                      SizedBox(
                        ...
                        child: FilledButton.icon(
                          ...
                          label: const Text('Start Monitoring', style: TextStyle(fontWeight: FontWeight.w800, fontSize: 16)),
                        ),
                      ),
                    ],
                  ),
            ...
            SliverPadding(
              ...
              sliver: SliverToBoxAdapter(
                child: Row(
                  children: [
                    Expanded(child: _metric('READY', 'AI engine', Icons.auto_awesome, const Color(0xFF4EA8FF))),
                    const SizedBox(width: 12),
                    Expanded(child: _metric('ON', 'Offline mode', Icons.wifi_off_rounded, const Color(0xFF27D77F))),
                  ],
                ),
              ),
            ),
              ...
              sliver: SliverToBoxAdapter(
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(color: const Color(0xFF172F44)),
                  ),
                  child: const Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
        ...
        Expanded(
          child: Column(
            ...
            children: [
              ...
              Text(label, style: const TextStyle(color: Color(0xFF7F96AA), fontSize: 11)),
            ],
          ),
        ),
      ],
...
class _MonitoringScreenState extends State<_MonitoringScreen> {
  ...
  String _driver = 'Safe';
  String _alertCause = 'No active warning';
  String _emergencyStatus = 'Standby';
  double _left = 1;
  double _right = 1;
  double _mouth = 0;
  double _headX = 0;
  ...
  Future<void> _captureAndAnalyze() async {
    ...
    try {
      ...
      // Camera/ML Kit frames can fail transiently; keep monitoring alive.
    } finally {
      _processing = false;
    }
  }
  ...
  Future<void> _registerAlert(String type, String message) async {
    ...
    if (mounted) {
      setState(() => _emergencyStatus = _emergencyStarted ? 'Emergency triggered' : 'Alert $_alerts / 3');
    }
  }
  ...
  Future<void> _sendEmergencyMessage(String message) async {
    ...
    if (mounted) {
      setState(() => _emergencyStatus = sent > 0 ? 'Emergency message sent' : 'Emergency message failed');
    }
  }

  Color get _statusColor {
    if (_driver == 'Safe') return const Color(0xFF27D77F);
    if (_driver == 'Emergency') return const Color(0xFFFF4D5E);
    return const Color(0xFFFFB84D);
  }

  Widget _signal(String title, String value, IconData icon, Color color) {
    return Container(
      padding: const EdgeInsets.all(15),
      decoration: BoxDecoration(
        color: const Color(0xFF0B1B2A),
        borderRadius: BorderRadius.circular(18),
        border: Border.all(color: const Color(0xFF18364D)),
      ),
      child: Row(
        children: [
          Icon(icon, color: color, size: 22),
          const SizedBox(width: 12),
          Expanded(child: Text(title, style: const TextStyle(color: Color(0xFFAEC0CE)))),
          ...
          Padding(
            ...
            child: Center(
              child: Text(
                _running ? '● LIVE' : '○ IDLE',
                style: TextStyle(
                  color: _running ? const Color(0xFF27D77F) : const Color(0xFF8AA0B3),
                  fontSize: 12,
      ...
      body: ListView(
        ...
        children: [
            ...
            child: _camera != null && _camera!.value.isInitialized
                ...
                : const Center(
                    child: Column(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        Icon(Icons.videocam_off_outlined, size: 48, color: Color(0xFF557083)),
                        SizedBox(height: 10),
                        Text('Camera is ready', style: TextStyle(color: Color(0xFF8EA5B7))),
                        SizedBox(height: 5),
                        Text('Start monitoring to begin AI analysis', style: TextStyle(color: Color(0xFF5F7587), fontSize: 12)),
                      ],
                    ),
                  ),
          ),
              ...
              children: [
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      const Text('SAFETY STATUS', style: TextStyle(fontSize: 10, color: Color(0xFF7F96AA), fontWeight: FontWeight.w800, letterSpacing: 1)),
                      const SizedBox(height: 5),
                      Text(_driver.toUpperCase(), style: TextStyle(fontSize: 23, color: _statusColor, fontWeight: FontWeight.w900)),
                      const SizedBox(height: 3),
                      Text(_alertCause, style: const TextStyle(color: Color(0xFF9BB0C1), fontSize: 11)),
                    ],
                  ),
                ),
                Text('$_alerts', style: TextStyle(fontSize: 28, color: _statusColor, fontWeight: FontWeight.w900)),
              ],
            ),
          ),
          ...
          Container(
            ...
            decoration: BoxDecoration(
              color: _emergencyStarted ? const Color(0x33FF4D5E) : const Color(0xFF0B1B2A),
              borderRadius: BorderRadius.circular(16),
              border: Border.all(color: _emergencyStarted ? const Color(0x66FF4D5E) : const Color(0xFF18364D)),
            ),
            child: Row(
              children: [
                Icon(_emergencyStarted ? Icons.emergency : Icons.shield_outlined, size: 19, color: _emergencyStarted ? const Color(0xFFFF4D5E) : const Color(0xFF27D77F)),
                const SizedBox(width: 10),
                Expanded(child: Text(_emergencyStatus, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w700, color: _emergencyStarted ? const Color(0xFFFF7A85) : const Color(0xFF9BB0C1)))),
                const Text('3 alerts', style: TextStyle(fontSize: 11, color: Color(0xFF71899B)),),
              ],
            ),
          ),
          ...
          const SizedBox(height: 10),
          _signal('Yawning', _yawn, Icons.face_retouching_natural_outlined, _yawn == 'No' ? const Color(0xFF27D77F) : const Color(0xFFFFB84D)),
          const SizedBox(height: 10),
          _signal('Head position', _head, Icons.person_pin_circle_outlined, _head == 'Forward' ? const Color(0xFF27D77F) : const Color(0xFFFFB84D)),
          const SizedBox(height: 16),
          Row(
            children: [
              Expanded(child: FilledButton.icon(onPressed: _running ? null : _start, icon: const Icon(Icons.play_arrow_rounded), label: const Text('START', style: TextStyle(fontWeight: FontWeight.w800)))),
              const SizedBox(width: 12),
              Expanded(child: OutlinedButton.icon(onPressed: _running ? _stop : null, icon: const Icon(Icons.stop_rounded), label: const Text('STOP', style: TextStyle(fontWeight: FontWeight.w800)))),
            ],
          ),
          ...
          Text(
            ...
            style: const TextStyle(color: Color(0xFF5F7587), fontSize: 10),
          ),
        ],
      ),
    );
