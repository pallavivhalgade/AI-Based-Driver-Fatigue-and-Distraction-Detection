...
class _DashboardScreenState extends State<DashboardScreen> {
  ...
  static const MethodChannel smsChannel = MethodChannel('auto_sms_channel');
  String smsStatus = 'Ready';
  String telegramStatus = 'Ready';
  String appMode = 'Offline Detection';
  bool emergencySmsProcessStarted = false;

  static const String telegramBotToken = '8328978924:AAF2amSV-pY7CNuuVbS1HhASzWmQ8JPZjj4';
  static const String telegramChatId = '5580786238';
  ...
  Future<void> updateHybridStatus(Face face, List<double> scores) async {
    ...
    if (alert) {
      ...
      emergencySmsProcessStarted = false;
      smsStatus = 'Ready';
      telegramStatus = 'Ready';
      appMode = 'Offline Detection';
    }
  ...
  Future<bool> sendTelegramEmergencyAlert(String message) async {
    try {
      final url = Uri.parse(
      ...
      setState(() {
        telegramStatus = ok ? 'Telegram Sent' : 'Telegram Failed';
        appMode = ok ? 'Online + Offline Backup' : 'Offline Fallback';
      });

      await LocalStore.addHistory(
        ok ? 'Telegram Alert Sent' : 'Telegram Alert Failed',
        ok ? 'Online Telegram emergency alert sent successfully' : 'Telegram server returned ${response.statusCode}',
      );
      ...
      setState(() {
        telegramStatus = 'Telegram Offline';
        appMode = 'Offline Fallback';
      });
      await LocalStore.addHistory('Telegram Offline', 'Internet unavailable or Telegram request timed out');
      return false;
    }
  }

  Future<void> triggerEmergencySms() async {
    if (emergencySmsProcessStarted) return;
    emergencySmsProcessStarted = true;
    ...
    if (contacts.isEmpty) {
      if (mounted) {
        setState(() => smsStatus = 'No contacts saved');
      }
      await LocalStore.addHistory('Emergency SMS Failed', 'No emergency contacts saved');
      return;
    }

    final message =
        'Emergency Alert: Driver fatigue detected. The driver ignored 3 alerts. Please contact the driver immediately.';

    await sendTelegramEmergencyAlert(message);
    ...
    if (!mounted) return;
    setState(() {
      smsStatus = failCount == 0 ? 'Emergency SMS Triggered' : 'SMS Partial/Failed';
    });

    await LocalStore.addHistory(
      'Emergency SMS',
      'SMS process completed. Success: $successCount, Failed: $failCount',
    );
  }
    ...
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          ...
          statusCard('Mode', appMode, Icons.swap_horiz, valueColor(appMode)),
          statusCard('Telegram', telegramStatus, Icons.telegram, valueColor(telegramStatus)),
          ...
          statusCard('Emergency SMS', smsStatus, Icons.sms, valueColor(smsStatus)),
        ],
      ),
    );
  }
